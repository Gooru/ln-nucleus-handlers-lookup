...
class SchoolHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (keyword != null && !keyword.isEmpty() && schoolDistrictId != null) {
      result =
          AJEntitySchool.where(LIST_SCHOOL, keyword + HelperConstants.PRECENTAGE, schoolDistrictId)
              .limit(limit).offset(offset).orderBy(HelperConstants.NAME);
    } else if (keyword != null && !keyword.isEmpty()) {
      result = AJEntitySchool.where(LIST_SCHOOL_FLT_BY_QUERY, keyword + HelperConstants.PRECENTAGE)
          .limit(limit).offset(offset).orderBy(HelperConstants.NAME);
    } else if (schoolDistrictId != null) {
      result = AJEntitySchool.where(LIST_SCHOOL_FLT_BY_SCHOOL_DISTRICT, schoolDistrictId)
          .limit(limit).offset(offset).orderBy(HelperConstants.NAME);
    } else {
