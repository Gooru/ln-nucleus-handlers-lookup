...
class FirebaseJwtCreationProcessor extends AbstractCommandProcessor {
  ...
  /*
   * Generate a JWT using information already contained in the Auth-Gateway token. Since we are
   * receiving the token, there is no need for us to authenticate the user again. Instead we
   * generate the JWT based off of the user's uid and send it back to the front end. Currently there
   * are no permissions associated with the token (though we can add "premiumAccount" if needed) and
   * instead the permissions to the DB are set in the DB rules.
   */
  @Override
  protected MessageResponse processCommand() {
    ...
    additionalClaims.put(MessageConstants.CLAIM_USERNAME,
        context.session().getString(MessageConstants.CLAIM_USERNAME));
